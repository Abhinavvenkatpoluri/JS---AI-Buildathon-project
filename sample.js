...
export async function main() {
  ...
  const response = await client.path("/chat/completions").post({
    body: {
      messages: [
        { role: "system", content: "You are a frontend developer and you write code in html and css" },
        { role: "user", content: [
            { type: "text", text: "take reference from  image and write code in html and css"},
            { type: "image_url", image_url: {
                url: getImageDataUrl("sketch.jpg", "jpg"), details: "low"}}
          ]
        }
      ],
      model: modelName
