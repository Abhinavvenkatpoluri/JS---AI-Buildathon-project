...
export class ChatInterface extends LitElement {
  static get properties() {
    return {
      ...
      isRetrieving: { type: Boolean }, // new property
      ragEnabled: { type: Boolean } // new property
    };
  }
      ...
      <div class="chat-container">
        <div class="chat-header">
          ...
          <label class="rag-toggle">
            <input type="checkbox" ?checked=${this.ragEnabled} @change=${this._toggleRag}> Use Employee Handbook
          </label>
        </div>
  ...
  async _sendMessage() {
    ...
    this.inputMessage = '';
    this.isRetrieving = this.ragEnabled; // show handbook search if enabled
    this.isLoading = true;

    try {
      const data = await this._apiCall(userQuery);
